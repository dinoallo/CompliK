//nolint:testpackage // Tests exercise unexported worker helpers.
package pagereview
...
func TestClaimUsesAdminTaskContract(t *testing.T) {
	...
	client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			...
			Body: io.NopCloser(
				strings.NewReader(
					`{"tasks":[{"id":42,"namespace":"ns-demo","ingress_name":"web","host":"example.com","path":"/docs","url":"https://example.com/docs"}]}`,
				),
			),
	...
	if err := worker.loadConfig(
		`{"adminBaseURL":"http://admin.example","adminTimeoutSecond":2,"leaseDurationSecond":30,"reviewTimeoutSecond":20,"adminBasicAuthUsername":"user","adminBasicAuthPassword":"pass"}`,
	); err != nil {
	...
	if received.WorkerID != "worker-1" || received.Limit != defaultBatchSize ||
		received.LeaseDurationSecond != 30 {
...
func TestDetectorEventSignalsPendingTask(t *testing.T) {
	...
	worker := &Plugin{done: workerDone, pending: make(map[uint64]chan struct{})}
