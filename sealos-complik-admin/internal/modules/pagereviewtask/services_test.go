...
func (f *fakeTaskRepository) Enqueue(_ context.Context, task *PageReviewTask, _ time.Time) (*PageReviewTask, bool, error) {
	f.enqueued = task
	copy := *task
	copy.ID = 17
...
func (f *fakeTaskRepository) Claim(context.Context, string, int, time.Duration, time.Time) ([]PageReviewTask, error) {
	return nil, nil
}

func (f *fakeTaskRepository) Complete(context.Context, uint64, string, time.Time, time.Duration) error {
	return nil
}
...
func TestNormalizeTaskRejectsURLOutsideRouteHost(t *testing.T) {
	...
	if err != ErrPageReviewTaskInvalidInput {
